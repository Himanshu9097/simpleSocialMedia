...
const Messages = () => {
    ...
    useEffect(() => {
        const fetchConnections = async () => {
            try {
                ...
                const { data } = await axios.get('http://localhost:3000/api/users/connections/all', {
    ...
    const fetchMessages = async () => {
        try {
            const token = localStorage.getItem('token');
            const { data } = await axios.get(`http://localhost:3000/api/messages/${activeChat._id}`, {
                headers: { Authorization: `Bearer ${token}` }
            });
    ...
    const handleSendMessage = async (e) => {
        ...
        try {
            const token = localStorage.getItem('token');
            const { data } = await axios.post(`http://localhost:3000/api/messages/send/${activeChat._id}`,
                { text },
                { headers: { Authorization: `Bearer ${token}` } }
            );
